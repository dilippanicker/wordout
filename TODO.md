...
- ✅ **Backfilled session 35's missing handoff** — that session ended without running `/close`: a version bump to v1.7.3, a CI fix for a failed Android SDK setup step, and the resulting GitHub Release were never captured. Reconstructed from `git log` and GitHub Actions history; committed as `chore(backfill): reconcile session 35 (v1.7.3 release, CI fix) into TODO.md and session-handoff.md` (`155b344`).
- ✅ **Fixed single-board (1-out) practice not remembering its own last-played difficulty** — user reported daily's difficulty carrying forward into practice. Root cause: `gameStore` had no equivalent to `quordleStore.lastDifficultyByBoardCount` — entering 1-out practice from elsewhere (quordle board-count cycling, Daily→Practice tab) just inherited whatever `settingsStore.difficulty` happened to be left at. Added `gameStore.lastDifficulty`, updated on every `switchDifficulty()` call, restored at all three entry points (`index.tsx`'s `cycleTo`, `settings.tsx`'s `handleBoardCountSelect`, the Daily→Practice tab). Also caught and fixed a related gap during review: the Settings difficulty picker's "browsing daily" branch bypassed `switchDifficulty` entirely, which would have gotten silently overwritten by the new restore logic — merged it into the same snapshot-aware path. Verified via `tsc`, jest (new regression test), and live browser: 1-out correctly held Extreme through round-trips via 6-out, which independently kept Hard; the Settings-panel-while-in-Daily path still works. Commit `3d670a3`.
- ✅ **Fixed language switch (`en_gb`↔`en_us`) wiping the current board** — user reported: solving an n-out game, switching language and back cleared it. Same root-cause pattern as the difficulty-switch bugs — both `gameStore` and `quordleStore` treated any language change as a full reset instead of a snapshot-aware switch. Extended both stores' snapshot keys to include language and added `switchLanguage(oldLanguage, newLanguage)` to each, called from the `settingsStore` subscribe (the only place both old and new language values are available, since nothing calls it directly the way difficulty/board-count switches do). Hit and fixed a real bug in my own test methodology along the way — see REGRESSION_TRAPS.md's new "Language switch wiped the other language's board" entry for the double-invocation gotcha. Verified via `tsc` and jest (2 new regression tests, 55/55 passing); live browser verification was attempted but the Chrome extension tab became unresponsive and was abandoned in favor of the automated verification. Commit `4d86452`.
- ✅ **Swapped New Game and Dark/Light Mode header icons** — user reported accidentally hitting New Game (which triggers the abandon-confirm dialog) while trying to tap the adjacent difficulty toggle. Discussed the fix approach first (also considered and declined adding a Color Blind Mode header shortcut — kept in Settings only, low toggle frequency didn't justify the added header clutter). Swapped ↺ New Game with ☽ dark/light mode: the difficulty icon's new neighbor is now a cheap, reversible action instead of a destructive one. Updated the in-app Help screen (`constants/helpContent.ts` + `components/HelpModal.tsx`'s parallel icon-render array) and README.md to match the new order. Commit `b283770`.
- All four commits pushed to origin/main at user's explicit request.
...
## ✅ Session 30 — Completed 2026-07-27 (daily-refresh-on-resume fix, swipe-to-cycle feature, v1.7.0 release)

- ✅ **Daily refresh on app resume from background (bugfix)** — user reported: left running overnight, reopened still showed yesterday's completed daily boards + "next word in ~22h" countdown; only full app restart picked up new day's words. Root cause: `useFocusEffect` only fires on in-app navigation focus, never on OS-level background→foreground resume. Fix: created `refreshDailyDay()` helper that calls `checkAndReset()` then `startOrResumeDailyGame()` (checkAndReset alone leaves active difficulty unplayable). Wired into three independent triggers: 1s countdown interval (day-roll while foregrounded), useFocusEffect (in-app nav focus), new AppState 'change' listener (OS-level resume). Verified live: reproduced exact bug in browser dev session (faked stale state via debug hook), confirmed "Next word in 22h 💀" display, dispatched synthetic visibilitychange event, confirmed UI flipped to "Today's · Easy 🐣" with fresh answer (no page reload). Added regression test: "checkAndReset alone leaves active difficulty unplayable — callers must also call startOrResumeDailyGame". Documented pattern in CLAUDE.md Daily Gate Architecture.
- ✅ **Swipe-to-cycle daily difficulty on finished board (new feature)** — swiping left/right on a finished daily board cycles Easy/Hard/Extreme using the same accessible-list gate as header-emoji tap. Extracted pure logic to `utils/dailyDifficultyCycle.ts` (DIFFICULTY_CYCLE, accessibleDailyDifficulties, stepDifficulty) to enable unit testing (importing route file in Jest fails with reanimated native-init error). New test file `__tests__/daily-difficulty-cycle.test.ts` (7 tests): accessible-list gating, forward/backward wraparound, single-entry dead-end (returns null, no toast — bug caught by advisor review). Touch handlers (onTouchStart/onTouchEnd, 50px threshold) on board wrapper ONLY when `isDaily && activeGameStatus !== 'playing'` — asymmetry is intentional, confirmed with user (swipe on same surface as active typing risks accidental navigation; tap on separate header target is deliberate). Verified live: synthetic touch events confirm swipe-left advances, swipe-right wraps backward, and swiping from finished→auto-starting board deactivates handlers correctly.
- ✅ **v1.7.0 (versionCode 36) committed and released** — minor bump for new swipe feature + bugfix (not just patch). Two commits: `99838fd` (feat: swipe + daily-refresh fix; index.tsx, new utils/dailyDifficultyCycle.ts, test files, CLAUDE.md update) + `aa5036d` (chore: version bump; app.json, CHANGELOG.md, CLAUDE.md current-version line). Both pushed to origin/main. Triggered build-apk.yml: test 33s, build 47m3s, deploy-web 42s — all passed. GitHub Release `v1.7.0` published with wordout.apk/wordout.aab. Verified deploy-web.yml auto-fired on version-bump commit landing on main; deploy-itchio-web succeeded — GitHub Pages, itch.io Android, itch.io Web (HTML5) all confirmed current on v1.7.0. Refreshed local releases/wordout-latest.{apk,aab} via gh release download; byte sizes match GitHub release assets exactly.
- ✅ **User device test results** — tested v1.6.0 on Android device: all OK except daily-background-refresh bug (now fixed in v1.7.0), which clears v1.6.0's daily-auto-advance as device-verified. Uploaded v1.7.0 to Play Store closed testing and device-tested: all good. Updated CLAUDE.md Play Store bullet: v1.7.0 (vc36) uploaded 2026-07-27 and device-tested, superseding v1.6.0 (vc34); v1.6.1 (vc35) was never uploaded on its own (Play Store track skipped v1.6.0→v1.7.0). Soft flag: user's test confirms general app health but didn't explicitly exercise new swipe gesture on real device (only web-verified by me).
- ✅ **itch.io duplicate-file discovery** — user's screenshot showed TWO Android APK files: wordout.apk (correct, ~54 min ago) and wordout-latest.apk (93MB, stale). Root cause: build-apk.yml pushes ./wordout.apk; make.sh only pushes :html5 channel; neither ever produced wordout-latest.apk on itch.io (must be from a one-off manual `butler push releases/wordout-latest.apk` in the past — butler names uploads after local filename). User to delete manually via itch.io Edit Game → Download page (not yet done, open item). Added caution to CLAUDE.md itch.io Android bullet to prevent recreation.
