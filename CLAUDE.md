...
**Swipe-to-cycle**: on a *finished* daily board (won or lost) only, swiping left/right on the board also cycles through this same accessible list (`utils/dailyDifficultyCycle.ts`'s `stepDailyDifficulty`), left = forward, right = backward with wraparound. Deliberately NOT active mid-game (`activeGameStatus === 'playing'`) — swipe touch handlers share the board's surface with the tap-to-clear-rightward gesture, so an always-on swipe risked a mistimed tap during active typing being misread as a swipe and yanking the player to a different difficulty mid-guess. Tap (header emoji) has no such risk since it's a separate target, so it stays available anytime. This asymmetry was raised and explicitly confirmed as the preferred UX, not an oversight — do not "fix" it into uniform availability without asking first.
...
- Quordle: confirmAbandon if in-progress; free switch otherwise — snapshot-based via `quordleStore.switchDifficulty(d)`, mirroring single-board's `gameStore.switchDifficulty`. A finished board is preserved, not wiped, when cycling difficulty and back — only explicit New Game clears it. Snapshots are keyed by `` `${difficulty}-${boardCount}` `` (`quordleStore.ts`'s `snapshotKey()`), a second dimension added on top of the pre-existing board-count-only keying used by `switchBoardCount`. Both the confirmAbandon path and the free-switch path in `handleDifficultyToggle()` must call `useQuordleStore.getState().switchDifficulty(next)` **before** `setDifficulty(next)` — same ordering requirement as single-board — so the save step reads the outgoing difficulty from settingsStore before it changes. `switchDifficulty`'s fresh-board branch computes `maxGuesses` explicitly via `maxGuessesForDifficulty(newDifficulty, boardCount)` rather than through `initialState()`, since `initialState()` reads difficulty from settingsStore and would still see the old value at that point in the call order. (This supersedes a prior locked decision that a completed quordle board should advance straight to the next difficulty with a new game — see REGRESSION_TRAPS.md v1.5.9 for the maxGuesses/newGame() constraint this fix still preserves.)
...
**Current version:** `1.7.2` (versionCode 38)
...
- **Reapplication still blocked as of 2026-08-17** — Google's stated reason is more device testing required, but recruiting additional testers beyond the existing pool has stalled (no organic growth channel). Next reapplication attempt needs either a wider tester-recruitment push (e.g. word-puzzle communities, asking existing testers to keep the app opened/installed through the full 14-day window) or clarification from the pending support ticket on what "continuity" actually requires.
...
- **Card sizing / letterboxing (`constants/layout.ts`, `app/_layout.tsx`):** on web (always) and native large screens (≥600dp smallest dimension — Android 16+ ignores our portrait lock there since targetSdk is 36), the game renders as a fixed-size card (`WEB_CARD_MAX_WIDTH` 430 / `WEB_CARD_MAX_HEIGHT` 932 — iPhone 14 Pro Max's logical size, chosen to sit just above the tallest realistic phone viewport so it never clips real mobile users) centered on a `#1a1a1a` backdrop, instead of stretching full window size. The gate is `shouldLetterbox(w, h)` — the **single source of truth used by both `_layout.tsx` (card styling) and `index.tsx` (screenW/screenH clamps)**; never re-derive the condition locally or the two go out of sync (boards overflow, paging offsets misalign). Native phones (< 600dp min dimension) are provably unaffected — they keep plain `flex:1` full-bleed. Regression tests: `__tests__/layout.test.ts`. Below the cap (any real phone-sized web window, or an undersized iframe like itch.io's — see below), the clamps are a no-op — the card instead draws a theme-matched 1px border so there's still a visible edge against the surrounding page with no dark backdrop space required (also needed on native tablets in dark theme, where card `#121213` barely contrasts with the `#1a1a1a` backdrop and RN shadow doesn't render on Android). Native-tablet rendering is web-verified only (same code path), not device-verified.
...
- **itch.io — Android (APK):** automated — `build-apk.yml` pushes `./wordout.apk` via `butler` to the `:android` channel after every build (`ITCHIO_API_KEY` secret). Butler installs from `https://broth.itch.zone/...` — **not `.ovh`, which is retired and no longer resolves** (see REGRESSION_TRAPS.md). Never manually `butler push` a locally-named file (e.g. from `releases/`) straight to this channel — butler names the itch.io download after the local file's basename, and anything other than `wordout.apk` creates a second, permanent duplicate entry the automated pipeline never updates or cleans up (found 2026-07-27: a stale `wordout-latest.apk` sitting alongside the real one — needs manual deletion on itch.io's Edit Game page).
- **itch.io — Web (HTML5):** automated — `deploy-web.yml`'s `deploy-itchio-web` job exports a *separate, root-relative* web build (baseUrl unset), post-processes it with `scripts/itchio-postprocess.py`, and pushes to the `:html5` channel via `butler`. `./make.sh deploy-web` does the same locally. The postprocess script pins `<base href>` to the real CDN directory *before* normalizing `window.location` via `history.replaceState()` — order matters, see REGRESSION_TRAPS.md. It also adds `viewport-fit=cover` to the viewport meta tag (itch-export-only, not applied to the GitHub Pages export) so `env(safe-area-inset-*)` can activate on notched phones when itch's mobile-friendly embed renders full-screen — best-effort, unverified against a real notch. Caveat: a manual reload on a sub-route (e.g. `/settings`) 404s — inherent to client-routed SPAs on a static host, acceptable since itch.io embeds in an iframe with no reload affordance.
- **itch.io project embed settings** (`Edit game` page, not in git — check itch.io directly if it looks wrong): size **430×820**, "fullscreen button" enabled. Kept at or below the 430×932 web card cap so the embed always renders full-bleed (no dark backdrop) — a boundary is instead drawn in code (`app/_layout.tsx`'s `webCard` border) rather than by oversizing the iframe, since itch.io has no responsive/auto-fit sizing for non-Unity HTML5 and any size taller than 932 risks forcing the surrounding itch.io page to scroll before the keyboard is reachable. See REGRESSION_TRAPS.md for the two prior sizing attempts this replaced.
- **itch.io's fullscreen-toggle button overlaps our own 📊 stats icon**, since full-bleed puts both in the bottom-right corner and itch renders its button on top of the iframe (outside our app, not repositionable from our code). `components/BottomStrip.tsx`'s `isIframeEmbedded` (`window.self !== window.top`) reserves extra right-padding around the icon, but only while `useWindowDimensions()` is still at or under the web-card cap (430×932) — that's the only case where our card is actually edge-to-edge and the corner coincides with itch's button. In itch's real fullscreen the iframe expands to the full screen size, so the card reverts to its normal centered/backdropped look with margin on all sides, nowhere near itch's corner icon — no reservation needed. See REGRESSION_TRAPS.md: an earlier attempt tried tracking `document.fullscreenElement` directly, on the assumption that itch's fullscreen state cascades into our own document via the iframe's `allowfullscreen` permission — confirmed live that it doesn't.
- **Samsung Galaxy Store / Amazon Appstore:** manual upload for now, automate later. Amazon requires icons at 512×512 and 114×114, and screenshots at one of a fixed set of exact dimensions (800×480 … 2560×1600, portrait or landscape) — arbitrary sizes are rejected, so our 1080×2160 itch.io screenshots don't qualify as-is. `store-assets/amazon/` holds icons regenerated from `assets/icon.png` plus the itch.io screenshots letterboxed to 1080×1920 (closest matching width; scaled to fit and padded with the `#1a1a1a` backdrop color rather than cropped, so no gameplay content is lost).
