...
- **itch.io's fullscreen-toggle button overlaps our own 📊 stats icon**, since full-bleed puts both in the bottom-right corner and itch renders its button on top of the iframe (outside our app, not repositionable from our code). `components/BottomStrip.tsx`'s `isIframeEmbedded` (`window.self !== window.top`) reserves extra right-padding around the icon — dropped again once `document.fullscreenElement` goes truthy (itch's iframe has `allowfullscreen`, so its fullscreen state cascades into our own document), since the overlay button relocates/hides once actually fullscreen.
